...
exports.checkValues = function (DWInputValues) {
	...
	// 	this.checkValue(DWInputValues.Values, DWparameters.fieldNameCONTACT)
	// 	.then(success =>{
	// 		if(!success){
	// 			throw new Error('hay algun error aqui');
	// 		}
	// 		return success;
	// 	}
	// 	)
	...
	// 	})
	// })
	var invoiceNo = this.getFieldValue(DWInputValues.Values, DWparameters.fieldNameCONTACT);
	var invoiceDate = this.getFieldValue(DWInputValues.Values,DWparameters.fieldDOCUMENTDATE);
	...
	return new Promise((resolve, reject) => {
		...
		// 	return success;
		// })
 		this.isDuplicateInvoice(DWInputValues.FileCabinetGuid, invoiceNo)
 		.then(success => {
			if (!success) {
				throw new Error('There is already an invoice stored (#'+ invoiceNo +' from:'+ invoiceDate);
			}
...
// exports.checkValue = function (DWIndexFieldCollection,fieldName){
...
exports.getFieldValue = function (DWIndexFieldCollection, fieldName) {
	console.log(DWIndexFieldCollection);
	var field = DWIndexFieldCollection.find(x => x.FieldName == fieldName);
	if (field === undefined) {
		return ;
	}
...
exports.isDuplicateInvoice = function (fileCabinetGUID, invoiceNo, invoiceDate, supplierID) {
	return new Promise((resolve, reject) => {	
		...
		.end(function (response) {
			...
			.jar(CookieJar)
			.send({ 'Condition':[
				{ 'DBName':DWparameters.fieldNameCONTACT, 'Value': [invoiceNo] },
